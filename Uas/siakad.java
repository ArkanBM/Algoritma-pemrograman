    ...
    public static void selectionsort() {
        for (int i = 0; i < jumlahData - 1; i++) {
            ...
            for (int j = i + 1; j < jumlahData; j++)
                if (Mahasiswa[j].getNim().compareTo(Mahasiswa[datamin].getNim()) <= 1)
                    datamin = j;
            Mahasiswa temp = Mahasiswa[datamin];
            Mahasiswa[datamin] = Mahasiswa[i];
            Mahasiswa[i] = temp;
        }
        menu();
    }
